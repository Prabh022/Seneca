...
namespace seneca
{
	...
	int ProcessData::operator()(const std::string& target_file, double& avg, double& var) {
		...
		// Step 1: Compute average-factors using multiple threads
		std::vector<std::thread> threads;
		for (int i = 0; i < num_threads; ++i) {
			int start = p_indices[i];
			int end = p_indices[i + 1];
			int size = end - start;
			threads.emplace_back(computeAvgFactor, data + start, size, total_items, std::ref(averages[i]));
		}
		...
		// Step 2: Compute variance-factors using multiple threads
		threads.clear();
		for (int i = 0; i < num_threads; ++i) {
			int start = p_indices[i];
			int end = p_indices[i + 1];
			int size = end - start;
			threads.emplace_back(computeVarFactor, data + start, size, total_items, avg, std::ref(variances[i]));
		}
		for (auto& t : threads) t.join();
		...
		// Step 3: Save results to target file
		std::ofstream out(target_file);
		if (!out) {
			std::cerr << "Failed to open output file: " << target_file << std::endl;
			return -2;
		}
		out << "Average: " << avg << "\n";
		out << "Variance: " << var << "\n";
